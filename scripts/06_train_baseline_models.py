...
def extract_corr_neighbor_feature(snapshot: dict, node_idx: int, price_feature_dim: int, corr_type_ids: set[int]) -> np.ndarray:
    ...
    for corr_type_id in corr_type_ids:
        corr_mask |= edge_type == int(corr_type_id)
...
def build_samples(
    ...
    if FEATURE_SCHEMA_FILE.exists():
        with open(FEATURE_SCHEMA_FILE, "r", encoding="utf-8") as f:
            price_feature_dim = int(json.load(f)["non_news_feature_count"])
    else:
        price_feature_dim = 22
    corr_type_ids = {
        int(edge_type_map["corr_positive_top10"]),
        int(edge_type_map["corr_negative_top5"]),
        int(edge_type_map["sector_top5_only"]),
    }
    ...
    for snapshot_id, snapshot in enumerate(snapshots):
        ...
        for node_idx in target_indices:
            ...
            neighbor_feature = extract_corr_neighbor_feature(snapshot, node_idx, price_feature_dim, corr_type_ids)
...
def train_corr_gnn(
    ...
    train_weights[y_train > q75] = 2.0
    train_weights[y_train > q90] = 4.0

    x_train_t = torch.tensor(x_train, dtype=torch.float32)
    y_train_t = torch.tensor(y_train, dtype=torch.float32)
    train_weights_t = torch.tensor(train_weights, dtype=torch.float32)
    ...
    for epoch in range(1, GNN_EPOCHS + 1):
        ...
        for start in range(0, len(order), GNN_BATCH_SIZE):
            ...
            loss = (train_weights_t[batch_idx] * torch.abs(pred - y_train_t[batch_idx])).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
