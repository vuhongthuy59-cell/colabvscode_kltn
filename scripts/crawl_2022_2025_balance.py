...
OUTPUT_FILE = ROOT / "data" / "data_origial" / "Vietstock_News_2022_2025_crawl.xlsx"
...
def set_date_range_on_site(driver: webdriver.Chrome) -> None:
    """Set date range using simple JS value assignment."""
    script = """
        try {
            document.querySelector('#txtFromDate').value = arguments[0];
            document.querySelector('#txtToDate').value = arguments[1];
            document.querySelector('#txtFromDate').dispatchEvent(new Event('change'));
            document.querySelector('#txtToDate').dispatchEvent(new Event('change'));
        } catch(e) {}
    """
    try:
        driver.execute_script(script, START_DATE, END_DATE)
    except Exception:
        pass
...
def main() -> None:
    ...
    BALANCE = ["C4G","PHR","NT2","CSV","CRE","PPC","SIP","GVR","PAN","GEG","BSI","VGI","VRE","PLX","DCM","KDC","IJC","PVS","SZC","HDG","DPG","PC1","BWE","QNS","DGC","VCG","DPM","PVD","ORS","FTS","BSR","CTS","SSB","VND","TCB","VIX"]
    selected_tickers = load_tickers(limit=args.limit, only=",".join(BALANCE))
