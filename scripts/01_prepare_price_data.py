...
PRICE_QUALITY_COLUMNS = ["metric", "value", "note"]
...
def build_price_quality_report(prices: pd.DataFrame, features: pd.DataFrame) -> pd.DataFrame:
    invalid_open_close = (
    ...
    finite_returns = features["log_return"].replace([np.inf, -np.inf], np.nan).dropna()
    finite_volume_ratio = features["volume_ratio_20"].replace([np.inf, -np.inf], np.nan).dropna()

    rows = [
        ("price_rows", len(prices), "Rows after date filtering, ticker filtering, and duplicate removal"),
        ("feature_rows", len(features), "Rows used to build stock-level features"),
        ("duplicate_date_ticker_rows", int(prices.duplicated(["date", "ticker"]).sum()), "Must be 0 after cleaning"),
        ("missing_ohlcv_values", int(prices[["open", "high", "low", "close", "volume"]].isna().sum().sum()), "Must be 0 before feature engineering"),
        ("close_le_zero_rows", int(prices["close"].le(0).sum()), "Invalid price rows; should be 0"),
        ("open_le_zero_rows", int(prices["open"].le(0).sum()), "Invalid price rows; should be 0"),
        ("high_lt_low_rows", int(prices["high"].lt(prices["low"]).sum()), "Invalid OHLC range rows; should be 0"),
        ("open_or_close_outside_high_low_rows", int(invalid_open_close.sum()), "Open/close outside the daily high-low range"),
        ("volume_negative_rows", int(prices["volume"].lt(0).sum()), "Invalid volume rows; should be 0"),
        ("volume_zero_rows", int(prices["volume"].eq(0).sum()), "Zero-volume rows are kept but reported"),
        ("log_return_missing_rows", int(features["log_return"].isna().sum()), "Expected for first trading row per ticker"),
        ("rolling_vol_20_missing_rows", int(features["rolling_vol_20"].isna().sum()), "Expected warm-up rows before 20 observations"),
        ("rolling_vol_60_missing_rows", int(features["rolling_vol_60"].isna().sum()), "Expected warm-up rows before 60 observations"),
        ("volume_ratio_20_missing_rows", int(features["volume_ratio_20"].isna().sum()), "Expected warm-up rows before 20 observations"),
        ("abs_log_return_gt_30pct_rows", int(finite_returns.abs().gt(0.30).sum()), "Potential extreme daily moves or price-adjustment issues"),
        ("abs_log_return_gt_50pct_rows", int(finite_returns.abs().gt(0.50).sum()), "High-priority outlier review candidates"),
        ("volume_ratio_20_gt_5_rows", int(finite_volume_ratio.gt(5).sum()), "Large volume spikes relative to 20-day average"),
        ("volume_ratio_20_gt_10_rows", int(finite_volume_ratio.gt(10).sum()), "Extreme volume spikes relative to 20-day average"),
        ("abnormal_volume_rows", int(features["abnormal_volume"].fillna(0).astype(int).sum()), "Rows where volume_ratio_20 > 2"),
    ]
    return pd.DataFrame(rows, columns=PRICE_QUALITY_COLUMNS)
...
def main() -> None:
    ...
    write_csv(build_price_quality_report(prices, features), "price_quality_report.csv")
