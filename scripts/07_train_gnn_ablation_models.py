...
DATA_DIR = ROOT / "data"
OUT_DIR = local_output("07_gnn_ablation_models")
...
def train_model(
    ...
    q75 = float(np.quantile(y[train_mask], 0.75))
    q90 = float(np.quantile(y[train_mask], 0.90))
    train_weights = torch.tensor(sample_weights_from_train_quantiles(y[train_mask], q75, q90), dtype=torch.float32)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for start in range(0, len(order), BATCH_SIZE):
            ...
            loss = (train_weights[batch] * torch.abs(pred - y_train[batch])).mean()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    with torch.no_grad():
        predictions = {
            "train": model(torch.tensor(x[train_mask], dtype=torch.float32)).numpy(),
...
def main() -> None:
    ...
    relationship_edge_types = all_edge_types - {
